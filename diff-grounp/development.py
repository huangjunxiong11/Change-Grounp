        ...
        if os.path.exists(jpg_path):
            ...
            for key in Situations:
                if key == '.DS_Store' or key == '._.DS_Store':
                    continue
                dst_folder = jpg_path + '/' + key
                varible = os.listdir(dst_folder)
                for i in range(len(varible)):
                    oriName = '{}/{}'.format(dst_folder, varible[i])
                    reName = "{}/{}-{}-{:>02d}.jpg".format(dst_folder, Today, key, i)
                    os.rename(oriName, reName)
            return
        else:
